...
async def music_onoff(_, message: Message):
    ...
    if status in ("ON", "on", "On"):
        lel = await message.reply("tunggu sebentar...")if not message.chat.id in DISABLED_GROUPS:
...
"""
    ...
    if a.status != "administrator":
        await message.reply_text(
            """
Saya perlu menjadi admin dengan beberapa izin:- dapat mengelola obrolan suara: Untuk mengelola obrolan suara
...
"""
        ...
        file_name = path.join(path.realpath("downloads"), file_name)file = await convert(
...
""",
    ...
    if await is_active_chat(chat_id):
        position = await put(chat_id, file=file)_chat_ = (str(file)).replace("_", "", 1).replace("/", "", 1).replace(".", "", 1)
...
async def startyuplay(_, CallbackQuery):
    ...
    if smex > DURATION_LIMIT:
        await CallbackQuery.message.reply_text(
            f"""
Kesalahan DurasiDurasi yang Diizinkan: {DURATION_LIMIT}
...
"""
    ...
    thumb = await gen_thumb(thumbnail, title, userid, theme, ctitle)
    await mystic.delete()
    if await is_active_chat(chat_id):position = await put(chat_id, file=file)
        buttons = play_markup(videoid, user_id)
        _chat_ = (str(file)).replace("_", "", 1).replace("/", "", 1).replace(".", "", 1)
        cpl = f"downloads/{_chat_}final.png"
        shutil.copyfile(thumb, cpl)
...
async def popat(_,CallbackQuery): 
    ...
    try:
        ...
        ID3 = (result[2]["id"])
        ID4 = (result[3]["id"])
        ID5 = (result[4]["id"])ID6 = (result[5]["id"])
        ID7 = (result[6]["id"])
        ID8 = (result[7]["id"])
        ID9 = (result[8]["id"])
        ID10 = (result[9]["id"])                    
